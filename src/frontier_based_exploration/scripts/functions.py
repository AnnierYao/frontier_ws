...
def informationGain(mapData, point, r):
    # 添加调试信息
    rospy.loginfo(f"Calculating information gain for point {point} with radius {r}")
    
    infoGain = 0
    index = index_of_point(mapData, point)
    r_region = int(r/mapData.info.resolution)
    
    rospy.loginfo(f"Index: {index}, r_region: {r_region}")
    ...
    for n in range(0, 2*r_region+1):
        ...
        # 添加调试信息
        rospy.loginfo(f"Checking range: start={start}, end={end}, limit={limit}")
        
        for i in range(start, end+1):
            if (i >= 0 and i < limit and i < len(mapData.data)):
                # 添加调试信息
                if mapData.data[i] == -1:
                    point_coord = point_of_index(mapData, i)
                    dist = norm(array(point)-point_coord)
                    if dist <= r:
                        infoGain += 1
                        rospy.loginfo(f"Found unknown cell at index {i}, dist={dist}")
    
    result = infoGain*(mapData.info.resolution**2)
    rospy.loginfo(f"Final information gain: {result}")
    return result
# ________________________________________________________________________________
